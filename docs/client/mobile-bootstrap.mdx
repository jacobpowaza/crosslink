...
## The "Powered by Crosslink" badge

Every Crosslink-owned mobile screen carries the attribution, and the bootstrap
mounts it for you — pairing, onboarding, offline, revoked, and your own page
once it takes over. You write no markup for it. It sits at the foot of the
screen and links to the Crosslink repository.

Where it sits and what it looks like is yours:
...
| Field | What it does |
| --- | --- |
| `placement` | `top-left`…`bottom-right`, or `inline` to sit in the page flow. Default `bottom-center`. |
| `color` | Text colour of the badge. |
| `background` | Pill behind it. `transparent` for none. |
| `size` | Font size; a number is pixels. |
| `offset` | Distance from the chosen screen edges; a number is pixels. |
| `logoWidth` | Width of the wordmark. |
| `zIndex` | Stacking order, for a page with its own fixed furniture. |
| `className` | Extra class, for your own styling. |

There is no field that removes the badge or changes its wording. Colour, size
and placement are the whole surface. `writeStaticBootstrap()` takes the same
`attribution` object, so a published static origin looks the same as the one
your host serves.

<Note>
The desktop pairing card is the other side of this: it draws the Crosslink
wordmark in its own column and carries no attribution line. See
