...
<Note>
`source: true` points the card at the Crosslink system endpoints your host
already exposes (`/__crosslink/*`). You do not write those routes — see
[Mobile Bootstrap](/client/mobile-bootstrap) for how they get there.
</Note>
...
host directly, supplies its own source instead of `source: true`:
...
Omit `source` entirely and the card renders only what you pass to `update()`.
This exists for hosts that already own a pairing loop; it is not the normal
integration path, and choosing it means writing the loop the card would
otherwise run for you.

```js
const card = createPairingCard({ target: "#crosslink" });
