...
Opens remote access now, if it is not already open. Needed only when
`networkMode` becomes `"remote"` after `start()` — a settings toggle rather than
a constant. `getPairingCode(undefined, "remote")` calls it for you; call it
directly when the UI wants the diagnostics before it renders a QR.
