...
`mobile.attribution` configures the "Powered by Crosslink" badge the bootstrap
mounts on every Crosslink-owned mobile screen — `placement`, `color`,
`background`, `size`, `offset`, `logoWidth`, `zIndex`, `className`. It defaults
to the foot of the screen and there is no field that removes it. See
